...
export default function Process() {
  return (
    <section id="werkwijze" className="relative bg-background py-20 md:py-28">
      <div className="container-tdc">
        ...
        <div className="grid gap-6 md:grid-cols-12 md:gap-10 items-start">
          {/* Real polishing photo */}
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.7 }}
            className="md:col-span-5 relative overflow-hidden rounded-2xl border border-foreground/10 aspect-[4/5] md:aspect-auto md:h-[540px]"
          >
            ...
            <ol className="divide-y divide-foreground/10 border-y border-foreground/10">
              {processSteps.map((step, i) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontFamily: "'Inter Tight', sans-serif" }} className="text-2xl md:text-3xl font-extrabold tabular-nums text-foreground/20 shrink-0 w-14">
                      {step.n}
                    </span>
                    <span className="mt-1 grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-foreground/5 text-foreground transition-colors group-hover:bg-primary group-hover:text-white">
                      <Icon size={17} strokeWidth={1.75} />
                    </span>
                    <div>
                      <h3 style={{ fontFamily: "'Inter Tight', sans-serif" }} className="text-lg md:text-xl font-bold tracking-tight text-foreground">
                        {step.title}
                      </h3>
